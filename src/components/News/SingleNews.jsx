...
const SingleNews = (props) => {
    const {itemClass, courseID, courseTitle, newsDescription, newsDate, newsType} = props;
    useEffect(() => {
        console.log("data: ", newsDate, newsType)
    }, []);
    return (
        <div className={itemClass ? itemClass : 'course__item mb-30'}>
            ...
            <div className="course__inner">
                <h3 className="react-course-title text-truncate">
                    <Link to={`/course/${courseID}`}>
                        {courseTitle ? courseTitle : 'Dave conservatoire is the Entirely free online.'}
                    </Link>
                </h3>
                <h4 className={"fs-5 container d-flex justify-content-end align-items-end fw-semibold text-lowercase"}>
                ...
                <div className="container">
                    <p>
                        {newsDescription ? newsDescription : 'empyt'}
                    </p>
                </div>
                <div className="course__card-icon d-flex align-items-center">
                    <div className="course__card-icon--1">
                        ...
                        {/*  date shown here  */}
                        <span>{
                            new Date(newsDate ? newsDate : "2021-08-01").toLocaleDateString("en-IN")
                        }</span>
                    </div>
